...
class RestaurantServiceTest {
  ...
  void normalHourServingRadiusIs5Kms() throws IOException {
    ...
    when(restaurantRepositoryServiceMock.findAllRestaurantsCloseBy(any(Double.class), any(Double.class), any(LocalTime.class), any(Double.class)))
        ...
        GetRestaurantsResponse getRestaurantsResponse = restaurantService.findAllRestaurantsCloseBy(new GetRestaurantsRequest(20.0, 30.0), LocalTime.of(22, 0));

        assertEquals(4, getRestaurantsResponse.getRestaurants().size());
        assertEquals("10", getRestaurantsResponse.getRestaurants().get(0).getRestaurantId());
        assertEquals("11", getRestaurantsResponse.getRestaurants().get(1).getRestaurantId());
        assertEquals("12", getRestaurantsResponse.getRestaurants().get(2).getRestaurantId());

        ArgumentCaptor<Double> servingRadiusInKms = ArgumentCaptor.forClass(Double.class);

        verify(restaurantRepositoryServiceMock, times(1))
          .findAllRestaurantsCloseBy(any(Double.class), any(Double.class), any(LocalTime.class), servingRadiusInKms.capture());
